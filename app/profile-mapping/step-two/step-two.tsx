...
export default function StepTwo () {
  const { progress, handleSelectEnvironment, selectedEnvironment } = useStepTwoModel();
 
  return (
    <View className='flex flex-col w-full h-full items-center justify-between gap-4 p-4 bg-[#FDF6F5]'> 
      ...
      >
        <View className="flex-row flex-wrap gap-2 justify-center items-center">
          {enviromentItems?.map(( item ) => (
            <Button 
              key={item.id} 
              className={twMerge('flex items-center justify-center w-[48%] h-[110px]', selectedEnvironment.includes(item.id) ? 'border-4 border-[#FF724C]' : 'border border-transparent')}
