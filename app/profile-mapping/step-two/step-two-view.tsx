...
export default function StepTwo() {
  ...
  return (
    <View className="flex flex-col w-full h-full items-center justify-between gap-4 p-8 bg-[--primary-bg]">
      ...
      >
        <View className="flex-row flex-wrap gap-2 justify-center items-center">
          {placeTypes?.map(item => {
            const isSelected = selectedEnvironment.includes(item.id);
      ...
      <View className="w-full">
        <Button
          onPress={() => {
            setSelectedPlaceTypes(selectedEnvironment);
            router.push('/profile-mapping/step-three/step-three-view');
          }}
